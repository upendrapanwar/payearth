...
class SaveLaterlist extends Component {
    ...
    componentDidMount() {
        this.getSaveLaterlistData('didMount');
    }

    getSaveLaterlistData(param) {
        ...
        axios.post('user/savelaterlist/' + this.authInfo.id, reqBody, {
            headers: {
            ...
            if (response.data.status) {
                this.setState({
                    data: response.data.data.savelater,
                    totalSaveLaterlistCount: response.data.data.totalSaveLaterlists,
                    reqBody
                });
            }
        }).catch(error => {
    ...
    deleteItem(productId) {
        ...
        dispatch(setLoading({loading: true}));
        axios.post('user/remove-from-savelater/', reqBody, {
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json;charset=UTF-8',
                'Authorization': `Bearer ${this.authInfo.token}`
            ...
            if (response.data.status) {
                ...
                let totalSaveLaterlistCount = this.state.totalSaveLaterlistCount - 1;
                ...
                this.setState({data: filteredArray, totalSaveLaterlistCount});
                localStorage.setItem('selectedSaveLaterItems', JSON.stringify(filteredItems));
                dispatch(setSelectedSavedItems({selectedSaveLaterItems: filteredItems}));
    ...
    render() {
        ...
        const { totalSaveLaterlistCount, data } = this.state;
                                ...
                                <div className="cart wishlist">
                                    <div className="cart_wrap">
                                        <div className="items_incart">
                                            <span>{totalSaveLaterlistCount} Save later Items</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="cart_list cart_wrap">
                                            ...
                                            data.map((value, index) => {
                                                ...
                                                return <div className={`cl_items ${cls}`} key={index}>
                                                        ...
                                                        <div className="cl_pro_btn">
                                                            <Link title="Remove from list" to="#" onClick={() => confirmation('Confirmation', 'Are you sure you want delete this item from savelater?', () => this.deleteItem(value.productId.id))}>
                                        ...
                                        <div className="pt-4 pb-4">
                                            {data.length > 0 && data.length < totalSaveLaterlistCount ?
                                                <div className="col-md-12 more_pord_load_btn">
                                                    <Link to="#" onClick={() => this.getSaveLaterlistData('viewMore')} className="view_more">View More</Link>
                                                </div>
                                            : '' }

                                            {data.length === 0 ? <NotFound msg="No Items found in savelater." /> : ''}
                                        </div>
                                    </div>
                                </div>
                            </div>
...
export default connect(setLoading) (SaveLaterlist);